...
# function-based views with enhancements
@api_view(['POST'])
...
def update_air_quality(request):
    ...
    try:
        aqi_data = {
            ...
            # Add any other data needed for prediction
        }
        
        # Get weather data for better predictions
...
# New class-based views for sensor-based AQI calculation and ML prediction
class AirQualityUpdateView(APIView):
    permission_classes = [IsAuthenticated]
    
    def post(self, request):
        """Update air quality using sensor data and AQI_Calculator"""
        try:
            # Option to use API data if requested
            use_api = request.data.get('use_api', False)
            
            if use_api:
                # Use OpenWeatherMap API if requested
                user = request.user
                if not user.latitude or not user.longitude:
                    return Response({
                        'error': 'User location not available'
                ...
                # Get weather data for additional info
                weather = WeatherService.get_weather_data(user.latitude, user.longitude)
                if weather:
                ...
                aqi_data = {
                    ...
                    'pm25': air_quality['pm25'],
                    'pm10': air_quality['pm10'],
                    'category': 'Unknown',  # API doesn't provide this directly
                    'temperature': sensor_data.get('temperature'),
                    'humidity': sensor_data.get('humidity'),
                    'pressure': sensor_data.get('pressure')
                }
            else:
                # Use local sensor data and AQI_Calculator
                sensor_data = AQICalculatorService.read_sensor_data(use_simulated=True)
                
                # Calculate AQI values using AQI_Calculator.py
            ...
            # Make predictions using available ML models
            prediction_models = PredictionService.get_available_models()
            if not prediction_models:
                # Fallback to standard models if no models found
                prediction_models = ['ridge_model.pkl', 'random_forest_model.pkl']
            
            predictions = []
            for model_name in prediction_models[:2]:  # Use up to 2 models
            ...
            response_data = {
                **aqi_data,
                ...
                'data_source': 'OpenWeatherMap API' if use_api else 'Local Sensors'
            }
...
class AirQualityHistoryView(APIView):
    ...
    def get(self, request):
        ...
        try:
            # Get user's air quality history
...
class AirQualityDashboardView(APIView):
    ...
    def get(self, request):
        ...
        try:
            # Get latest air quality record from sensors
            ...
            # Get latest air quality data from API
            ...
            # Recent history from both sources
